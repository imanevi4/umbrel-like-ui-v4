  ...
  el.discoveryList.innerHTML = state.discoveryItems.map((item) => `
    <label class="discovery-item glass-subtle">
      <div class="discovery-row">
        <div>
          <input type="checkbox" data-discovery-item="${item.id}" />
          <strong>${escapeHtml(item.name)}</strong>
          <div class="muted small">${escapeHtml(item.image || '')}</div>
          <div class="muted small">${escapeHtml(item.url || item.internalUrl || '')}</div>
        </div>
        <div class="tag">${escapeHtml(item.category || 'Service')}</div>
      </div>
    </label>
  `).join('');
}
